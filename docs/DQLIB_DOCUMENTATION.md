...
| 市场风险 | 历史模拟曲线/即期情景生成、VaR、期望损失（ES） |
...
```dolphindb
caplib::createPricingSettings(currency STRING, pricingMethod STRING, incCurrent BOOL, cashFlows BOOL, specificPricingRequests INT[], modelHandle STRING, pdeHandle STRING, mcHandle STRING, handle STRING[, returnJson BOOL])
```
...
caplib::createPricingSettings(
    currency, "ANALYTICAL", 0, 0, take(0, 0), bsmModel, "", "", "EQ_BSM_ANALYTICAL", false)
```
